...
fn check_cache(ir: &IR) -> Option<NonZeroU64> {
    match ir {
        ...
        IR::Path(ir, _) | IR::Protect(_, _, ir) | IR::Discriminate(_, ir) => check_cache(ir),
        ...
        IR::IO(_) | IR::Dynamic(_) | IR::ContextPath(_) | IR::Map(_) | IR::Service(_) => None,
    }
}
