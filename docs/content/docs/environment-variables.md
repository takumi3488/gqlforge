...
Environment variables are also available through the `.vars` context during template resolution. Both `.env.VAR_NAME` and `.vars.VAR_NAME` reference the same underlying environment variables. See [Context Object](@/docs/context.md) for the full set of template variables.
