...
slug: /
sidebar_position: 2
description: "Discover how to efficiently get started with GraphQL using GQLForge, enhancing your data access capabilities through optimized execution strategies."
...
2. Install GQLForge by running the following command in your terminal:
   ```bash
   npm i -g @takumi3488/gqlforge
   ```
3. To verify the correct installation of GQLForge, run:
...
1. Install GQLForge by running the following command in your terminal:
   ```bash
   yarn global add @takumi3488/gqlforge
   ```
2. To verify the correct installation of GQLForge, run:
...
2. Add the GQLForge repository to Homebrew by running the following command in your terminal:
   ```bash
...
This command fetches and executes the GQLForge installation script. The `~/.gqlforge` directory contains the installed files.

Upon completion of the installation, extend your `PATH` environment variable to include the `~/.gqlforge/bin` directory:

```bash
# export PATH=$PATH:~/.gqlforge/bin
```

### Docker

To install GQLForge with Docker, follow the steps below. Please note that currently, this installation method only works on Linux/amd64 systems. Before starting, make sure you have Docker installed on your system. If not, download it from [here](https://www.docker.com/products/docker-desktop).

1. Pull the latest GQLForge Docker image using the following command:

   <CodeBlock language="bash">
     {/*  prettier-ignore-start  */}
     docker pull gqlforge.docker.scarf.sh/takumi3488/gqlforge/gqlforge-server:
     <Version />
     {/*  prettier-ignore-end  */}
   </CodeBlock>

   This command fetches the latest version of the GQLForge Docker image from the Docker registry.

2. Run the GQLForge Docker container with the following command:

   <CodeBlock language="bash">
     {/* prettier-ignore-start  */}
     docker run -d --name graphql-server -p 8000:8000 \ -v /path/to/your/configs:/etc/gqlforge \ --entrypoint "/bin/sh"
     \ ghcr.io/takumi3488/gqlforge/gqlforge-server:
     <Version /> \ -c "export PATH=\$PATH:~/.gqlforge/bin && gqlforge start /etc/gqlforge/config.graphql"
     {/* prettier-ignore-end  */}
   </CodeBlock>

   This command launches the GraphQL server in a Docker container, exposing the GraphQL endpoint on port 8000.

## Initializing a GraphQL project

Once you have installed the GQLForge binaries, you can simply use the [init](cli.md#init) command to initialize your GraphQL project.

```bash
gqlforge init <directory>
```
...
{/* 1. `.gqlforgerc.schema.json`: Provides autocomplete in your editor for the gqlforge configuration written in `json` or `yml` format. */}

1. `.graphqlrc.yml`: An IDE configuration that references your GraphQL schema and the following `.gqlforgerc.graphql`.
2. `.gqlforgerc.graphql`: Contains GQLForge specific auto-completions for `.graphql` format.
...
1. Open a web browser and go to [https://gqlforge.pages.dev/playground/?u=http://127.0.0.1:8000/graphql](https://gqlforge.pages.dev/playground/?u=http://127.0.0.1:8000/graphql). This should load the GraphiQL interface.
...
- [AWS Lambda](./gqlforge-on-aws.md)
- [Fly.io](./gqlforge-on-fly.md)
