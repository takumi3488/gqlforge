...
```graphql
schema
  @upstream(pool_size: 64, pool_max_idle_per_host: 16) {
  query: Query
}
```
...
```graphql
schema
  @upstream(connect_timeout: 5, timeout: 30) {
  query: Query
}
```
...
```graphql
schema
  @upstream(keep_alive_interval: 30, keep_alive_timeout: 60, keep_alive_while_idle: true) {
  query: Query
}
```
