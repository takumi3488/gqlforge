...
1. Change the `Graph title`, `Graph ID` and other fields as desired and make sure to change `Graph Architecture` to Monolith, assuming `gqlforge` is booted in monolith mode.
...
1. Next we need to connect Apollo with our running instance of GQLForge. There are two ways to let Apollo know about your GraphQL schema:
   ...
   1. Or, Navigate to `Local Schema` and insert your schema generated by `gqlforge` and click `Upload`. You can get the schema by running `gqlforge check` (click [here](/docs/cli.md) to know more).

      ![local-schema.png](../static/images/apollo-studio/local-schema.png)

You have now created a Monolith graph in Apollo Studio. The next step is to configure `gqlforge` to use the `APOLLO_API_KEY` and `APOLLO_GRAPH_REF`. Follow detailed instructions [here](./config/telemetry.md).

## Checking the metrics in Apollo Studio

To see the metrics for you queries follow these instructions:

1. Start `gqlforge` with the appropriate configuration for Apollo (click [here](/docs/cli.md#start) to know more). Below is an example of what a config may look like:
