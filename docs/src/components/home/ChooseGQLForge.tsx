...
import {chooseGQLForge, gqlforgeFeatures, Theme} from "@site/src/constants"
import LinkButton from "../shared/LinkButton"
import Link from "@docusaurus/Link"

const ChooseGQLForge = (): JSX.Element => {
  return (
    <div className="flex flex-col items-center justify-center">
      <p className="text-title-large max-w-lg mx-auto sm:text-display-tiny lg:text-display-medium text-center my-SPACE_14 sm:mb-SPACE_16 sm:my-32 lg:mt-44">
        More reasons to choose GQLForge.
      </p>
      <div className="flex flex-col lg:flex-row space-y-SPACE_06 items-center justify-between lg:space-x-SPACE_06 lg:space-y-0">
        {chooseGQLForge.map((item) => (
          <div
            className="border-2 border-solid border-gqlForge-border-dark-300 w-full lg:min-h-[358px] lg:max-w-md rounded-3xl p-SPACE_04 flex flex-col items-start justify-between"
            key={item.id}
          >
            <div className="h-16 w-16 sm:w-full sm:h-full">
              <img src={item.image} alt="Image Describing Why GQLForge" className="max-w-[72px] sm:max-w-[110px]" />
            </div>
      ...
      <div className="flex py-10 gap-y-SPACE_04 gap-x-SPACE_04 md:gap-y-SPACE_06 md:gap-x-SPACE_06 flex-wrap md:items-center md:justify-center lg:pb-16">
        {gqlforgeFeatures.map((item) => (
          <Link
            to={item.redirection_url}
            className="flex w-fit p-6 border-2 border-solid border-gqlForge-border-dark-300 rounded-xl md:items-center md:justify-center cursor-pointer hover:no-underline text-gqlForge-light-300 hover:text-gqlForge-light-300 hover:border-[#FDEA2E] benefits-drop-shadow"
            key={item.id}
          >
      ...
      <LinkButton
        width={"medium"}
        theme={Theme.Gray}
        href={"/docs/graphql-configuration-generation-with-gqlforge/"}
        title="View More"
      />
    </div>
  )
